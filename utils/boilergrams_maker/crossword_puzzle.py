...
class GridWord:
    ...
    def __contains__(self, item):
        if isinstance(item, str):
            ...
            return self.r1 <= item[0] <= self.r2 and self.c1 <= item[1] <= self.c2
        else:
...
class Grid:
    def __init__(self, num_rows=50, num_cols=50, seed=None):
        self.num_rows = num_rows
        self.num_cols = num_cols
        self.grid = np.full([self.num_rows, self.num_cols], "")
        self.grid_words: list[GridWord] = []
        if seed is None:
            seed = 0
    ...
    def __scan_and_insert_word(self, word):
        ...
        if len(self.grid_words) == 0:
            self.__insert_word(GridWord(word, *self.__approximate_center(), self.rng.choice(list(Direction))))
            return(None)
    ...
    def __randomly_insert_word(self, word):
        ...
        while num_iterations <= 10000:
            rand_r = self.rng.randint(0, self.num_rows - 1)
            rand_c = self.rng.randint(0, self.num_cols - 1)
    ...
    def crop(self):
        ...
        for grid_word in self.grid_words:
            cropped_word = GridWord(grid_word.word, grid_word.r1 - min_r, grid_word.c1 - min_c, grid_word.direction)
